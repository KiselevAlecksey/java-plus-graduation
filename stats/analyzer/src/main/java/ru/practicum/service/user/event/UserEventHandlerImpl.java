...
public class UserEventHandlerImpl implements EventHandler {
    ...
    public void handle(SpecificRecord event) {
        ...
        if (userEventOptional.isPresent()) {
            UserEvent existingEvent = userEventOptional.get();
            if (userEvent.getRating() > existingEvent.getRating()) {
                existingEvent.setRating(userEvent.getRating());
            }
            userEventRepository.save(existingEvent);
        } else {
